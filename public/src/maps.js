...
&mode=fastest;${mean};traffic:enabled`)
        ...
        .then(res => {
          if (mean == 'car') {
            $(`#${mean}-time`).html(
              moment.utc(res.response.route[0].summary.trafficTime * 1000).format('HH:mm'))
          } else {
            $(`#${mean}-time`).html(
              moment.utc(res.response.route[0].summary.baseTime * 1000).format('HH:mm'))
          }
        })
    }
  )
...
function getPosition(userPosition) {
  ...
  $("#address-search").on('click', (e) => {
    e.preventDefault();
    var geocodingParams = {
      searchText: $("#address-search-bar").val().replace(/(-)|(,)/g, '')
    };
    var geocoder = platform.getGeocodingService();
    let userpoint = `geo!${userPosition.coords.latitude},${userPosition.coords.longitude}`
    console.log($("#address-search-bar").val().replace(/(-)|(,)/g, '').split(' ').join('+'))
